...
public class InjectionPropertyFilter : Attribute, IActionFilter
{
    ...
    {
        var properties = context.Controller.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Where(x => x.GetCustomAttributes(typeof(InjectionServiceAttribute), true).Any());
